...
4. **Done (2026-07-22):** `tests/scipy_reference.py` mirrors the SciPy-comparable
   `fix_benchmark` cases through real `scipy.optimize.shgo` and emits the same
   JSON-lines shape (`python3 -m venv .venv && .venv/bin/pip install scipy`, then run
   with that interpreter). Reference values from scipy 1.18: rastrigin 4-D sobol
   n=1024×2 → fun 4.97, 53 minima, nfev 5627 (Rust: fun 0.0, 239 minima, nfev 25628 —
   more thorough at ~4.5× the evals, reconcile pool-size semantics during §4 work);
   constrained sphere → fun 0.5 both ✓; and SciPy's 6-D Delaunay at just n=512×2 took
   16.6 s (vs Rust KNN 6-D n=16384×2 at 1.2 s) — the qhull wall the KNN backend removes.
