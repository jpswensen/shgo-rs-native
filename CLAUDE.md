...
Only `tests/fixtures/end_to_end_tests.json` is tracked in git: the other fixture files
(`vertex_tests.json`, `cache_tests.json`, `minimizer_tests.json`,
`triangulation_tests.json`, `sobol_tests.json`) are absent from a fresh clone, so 8 of the
11 tests in `tests/cross_validation.rs` fail with "No such file or directory" until they
are regenerated — that is not a code regression. End-to-end fixtures are regenerated by
...
- `local_opt.rs`: NLopt wrapper plus a CMA-ES backend. NLopt handles are `!Send`, so a
  fresh optimizer is built per call. The algorithm is selected via
  `local_options.algorithm` (BOBYQA default) — the old separate
  `ShgoOptions.local_optimizer` field was removed in commit 70ec80d; when constraints
  exist and the chosen algorithm can't handle them, SHGO auto-upgrades to COBYLA with a
  warning. SHGO's `g(x) >= 0` convention is negated to NLopt's `fc(x) <= 0`. Both public
  entry points (`minimize_local`, `minimize_local_constrained`) share one `run_local`
  dispatcher: `LocalOptimizer::Cmaes` goes to `run_cmaes` (the `cmaes` crate, built with
  `default-features = false` to avoid plotters/FreeType), everything else to `run_nlopt`,
  so constraints are always registered with NLopt. `to_nlopt_algorithm()` returns
  `Option<Algorithm>` (`None` for `Cmaes`). The CMA-ES backend has no native bounds:
  `BoxMap` normalises each coordinate by its bound width and `reflect_into` folds
  out-of-box samples back (triangle wave), so every evaluation is feasible and boundary
  minima are reachable; the run seed is `cmaes_seed(base, x0)` so the parallel pool stays
